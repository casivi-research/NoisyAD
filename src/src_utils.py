...
def initialize_exp(params, *args, dump_params=True):
    ...
    phase = [
        f'baselr{params.base_lr}',
        f'p{params.nmb_prototypes[0]}_K{params.K}_J{params.J}',
        f'e{params.epochs}', f'b{params.batch_size}', f'r224x224',
        f'd{params.in_dim}x{params.out_dim}', time_prefix
    ]

    params.dump_path = os.path.join(params.dump_path,
                                    '_'.join(params.train_class), 
                                    f'seed_{params.seed}',
                                    '_'.join(phase))
    os.makedirs(params.dump_path, exist_ok=True)
    os.makedirs(os.path.join(params.dump_path, 'ckpts'), exist_ok=True)
